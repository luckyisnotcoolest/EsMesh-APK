/*
 * ==============================================================================
 *  EsMesh Node Firmware - ESP32-S3 Target (Arduino IDE Sketch)
 * ==============================================================================
 *  Board: ESP32S3 Dev Module / ESP32-S3-DevKitC-1
 *  Settings:
 *    - USB CDC On Boot: Enabled
 *    - Flash Size: 8MB (or 4MB/16MB according to module)
 *    - Partition Scheme: Default 8MB (or Minimal SPIFFS / 4MB with spiffs)
 *    - PSRAM: OPI PSRAM / QSPI PSRAM / Disabled (as per board)
 *
 *  Required Libraries (Install via Arduino IDE Library Manager):
 *    - painlessMesh by painlessMesh / Benoit Blanchon
...
void changedConnectionCallback() {
    Serial.printf("[Mesh] Connection Topology Changed. Nodes: %u\n", mesh.getNodeList().size() + 1);
...
void setup() {
    ...
    Serial.println("  Target: ESP32-S3 (USB CDC Enabled)");
    Serial.println("==========================================\n");
    ...
    if (config.isGateway && config.wifiSsid.length() > 0) {
        Serial.printf("[Gateway] Connecting to Wi-Fi router: %s\n", config.wifiSsid.c_str());
        mesh.stationManual(config.wifiSsid, config.wifiPassword);
    }

    if (MDNS.begin("esmesh-s3")) {
    ...
    Serial.printf("[EsMesh-S3] Node ID: 0x%08X ready.\n", mesh.getNodeId());
}
...
void loadConfiguration() {
    ...
    config.nodeName = preferences.getString("name", "ESM-S3");
